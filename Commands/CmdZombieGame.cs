...
namespace MCForge
{
    public class CmdZombieGame : Command
    {
        ...
        StartGame:
            ...
            string[] array2 = new string[] { "", "", "'s brain", "", " meet their maker", "", "", " see the dark side", "", " open source", "", "", " got nommed on", " got $nameifyed!" };
            Server.infection = true;
            if (Server.mainLevel.name.ToLower() == "inferno_con_el_diablo")
            {
                timer = new System.Timers.Timer(660000);
                timeMinute = DateTime.Now.Minute + 11;
            }
            else
            {
                            ...
                            if (player2.pos[1] / 32 == player1.pos[1] / 32 || player2.pos[1] / 32 == player1.pos[1] / 32 - 1 || player2.pos[1] / 32 == player1.pos[1] / 32 + 1)
                            {
                                if (player2.pos[2] / 32 == player1.pos[2] / 32 || player2.pos[2] / 32 == player1.pos[2] / 32 + 1 || player2.pos[2] / 32 == player1.pos[2] / 32 - 1)
                                {
                                    if (!infect.Contains(player2) || infect.Contains(player1) && !player2.referee && !player1.referee && player1 != player2)
                                    {
                                        ...
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " " + File.ReadAllText("text/infect/" + player1.name + ".txt") + " " + c.red + player2.name);
                                        }
                                        else if (player1.name.ToLower() == "genuine_imation")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " made zombie babies with " + c.red + player2.name + c.yellow + ". PROBLEM?!?");
                                        }
                                        else if (player1.name.ToLower() == "LenaDINNERTIME")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " thought " + c.red + player2.name + c.yellow + " was a sandwich");
                                        }
                                        else if (player1.name.ToLower() == "cazzar")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " is online? WTF?!?!?!? Hey, look! " + c.red + player2.name + c.yellow + " is a zombie!");
                                        }
                                        else if (player1.name.ToLower() == "1koakuma1")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " made ponies with " + c.red + player2.name + c.yellow + ".");
                                        }
                                        else if (player1.name.ToLower() == "snowl")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " dibby-dobbed on " + c.red + player2.name + c.yellow + ". PROBLEM?!?");
                                        }
                                        else if (player1.name.ToLower() == "chiefinstaff")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " shank'd " + c.red + player2.name + c.yellow + ".");
                                        }
                                        else if (player1.name.ToLower() == "foxararma")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " made an archive with " + c.red + player2.name + c.yellow + ".");
                                        }
                                        else if (player1.name.ToLower() == "flowfire")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " blew fire at " + c.red + player2.name + c.yellow + ".");
                                        }
                                        else if (player1.name.ToLower() == "rainbowninjaa")
                                        {
                                            Player.GlobalMessage(c.red + player1.Original + c.yellow + " sliced " + c.red + player2.name + c.yellow + " with a rainbow sword");
                                        }
                                        else if (array2[cazzar] == "")
                                        {
        ...
        public void changeLevel()
        {
            ...
            try
            {
                ...
                Player.GlobalMessage(" " + c.black + "Level Vote: " + Server.DefaultColor + level1 + ", " + level2 + " or random " + "(" + c.lime + "1" + Server.DefaultColor + "/" + c.red + "2" + Server.DefaultColor + "/" + c.blue + "3" + Server.DefaultColor + ")");
                ...
                String nextl = "main";
                bool DoARandomLevel = false;
                Server.s.Log(Convert.ToString(Server.RandomLevelVotes));
                if (Server.NoLevelVotes >= Server.YesLevelVotes)
                {
                    if (Server.RandomLevelVotes > Server.NoLevelVotes && Server.RandomLevelVotes > Server.YesLevelVotes)
                    {
                        DoARandomLevel = true;
                    }
                    nextl = level2;
                ...
                DoARandomLevel = false;
                Server.RandomLevelVotes = 0;
